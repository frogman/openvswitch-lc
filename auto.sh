...
for ((i=1; i<=${NUM}; i++)); do
    sudo ip addr add 10.0.0.`expr $i \* 2`/24 brd 10.0.0.255 dev br0;
done
...
for ((i=1; i<=${NUM}; i++)); do
    sudo arp -s 10.0.0.`expr $i \* 2 - 1` 08:00:27:85:ca:de
done

ping -c 1 192.168.58.1 >/dev/null&
...
for ((i=1; i<=${NUM}; i++)); do
    ping -c 1 10.0.0.`expr $i \* 2 - 1` >>ping_history.txt;
    sleep 2;
done

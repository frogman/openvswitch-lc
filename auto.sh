NUM=20 #num of hosts on every edge

sudo kill `cd /usr/local/var/run/openvswitch && cat ovsdb-server.pid ovs-vswitchd.pid`;
sudo rmmod openvswitch;
sudo modprobe -r openvswitch;
sleep 1;
make && sudo make install && sudo insmod datapath/linux/openvswitch.ko && sudo ovsdb-server --remote=punix:/usr/local/var/run/openvswitch/db.sock --remote=db:Open_vSwitch,manager_options --private-key=db:SSL,private_key --certificate=db:SSL,certificate --bootstrap-ca-cert=db:SSL,ca_cert --pidfile --detach; 

sleep 1;

sudo ovs-vsctl --no-wait init; sleep 1; sudo ovs-vswitchd --pidfile --detach; sleep 1;

sudo route del default gw 192.168.56.1; sudo route del default gw 192.168.59.1; 
sudo route add -net 239.0.0.0/24 eth1; 
sudo ifconfig eth2 0;
sudo ifconfig br0 192.168.59.10 up;

for ((i=1; i<=${NUM}; i++)); do
    sudo ip addr add 10.0.0.`expr $i + 2 \* $NUM`/24 brd 10.0.0.255 dev br0; #10.0.0.41-10.0.0.60 is on host3
done

sudo sysctl -w net.ipv4.neigh.default.gc_stale_time=600;
sudo sysctl -w net.ipv4.neigh.br0.gc_stale_time=600;

#crl
sudo arp -s 192.168.59.1 0a:00:27:00:00:03;
for ((i=1; i<=${NUM}; i++)); do
    sudo arp -s 10.0.0.`expr $i + 0` 08:00:27:85:ca:de; #10.0.0.1-10.0.0.20 is on host1
    sudo arp -s 10.0.0.`expr $i + $NUM` 08:00:27:ab:b6:a5; #10.0.0.21-10.0.0.40 is on host2
done

ping -c 1 192.168.59.1 >/dev/null&
